...
DINOv3 の重みは Meta のライセンス承認が必要。以下のページから申請する:

- **申請先**: Meta の DINOv3 リポジトリ（GitHub `facebookresearch/dinov3` の README にリンクあり）

### 2.2 ダウンロード対象

申請承認後、以下の URL からダウンロードする。
...
**step5 用（ConvNeXt Small 推奨）:**

```
https://dl.fbaipublicfiles.com/dinov3/dinov3_convnext_small/dinov3_convnext_small_pretrain_lvd1689m-296db49d.pth
```

**代替（VRAM が不足する場合に ConvNeXt Tiny）:**

```
https://dl.fbaipublicfiles.com/dinov3/dinov3_convnext_tiny/dinov3_convnext_tiny_pretrain_lvd1689m-21b726bb.pth
```
...
| モデル | ファイル名 | サイズ目安 |
|--------|-----------|-----------|
| ViT-B/16 | `dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth` | ~334 MB |
| ConvNeXt Tiny (29M) | `dinov3_convnext_tiny_pretrain_lvd1689m-21b726bb.pth` | ~112 MB |
| ConvNeXt Small (50M) | `dinov3_convnext_small_pretrain_lvd1689m-296db49d.pth` | ~192 MB |
| ConvNeXt Base (89M) | `dinov3_convnext_base_pretrain_lvd1689m-801f2ba9.pth` | ~340 MB |
| ConvNeXt Large (198M) | `dinov3_convnext_large_pretrain_lvd1689m-61fa432d.pth` | ~756 MB |
...
> **注意**: サーバーから外部ネットワークへのアクセスが制限されているため、ローカル PC でダウンロードした後に `scp` 等で転送する必要がある。
